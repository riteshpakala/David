...
extension Date {
    ...
    var asProperString: String {
        return Calendar.nyTimeDayProperFormatter.string(from: self)
    }
    
    var asProperFileString: String {
        return Calendar.nyTimeDayProperFileFormatter.string(from: self)
    }
    ...
    public static var today: Date {
        ...
        let date: Date = .init()//Calendar(identifier: .gregorian)
//            .dateBySettingTimeFrom(timeZone: Calendar.nyTimezone, of: Date()) ?? .init()
//
    ...
    var dayofTheWeek: Weekday {
        let dayNumber = Calendar.nyCalendar.component(.weekday, from: self)
...
extension Calendar {
    ...
    static var nyTimeDayProperFormatter: DateFormatter {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = Locale(identifier: "en_US_POSIX")
        dateFormatter.timeZone = Calendar.nyTimezone
    ...
    static var nyTimeDayProperFileFormatter: DateFormatter {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = Locale(identifier: "en_US_POSIX")
        dateFormatter.timeZone = Calendar.nyTimezone
        dateFormatter.dateFormat = "yyyy-MM-dd-HH-mm-ss"
...
//MARK: -- Stocks
extension Date {
    static var tradingIsClosed: Bool {
        !Date.today.validTradingDay || (today.closingHour <= today.timeComponents().hour)
    }
    
    static var tradingIsOpen: Bool {
        Date.today.validTradingDay && today.openingHour <= today.timeComponents().hour
    }
    ...
    static var nextTradingDay: Date {
        ...
        if  todaysHour < today.closingHour &&
            ...
            //print("[Date] today is valid trading")
            return today
        } else {
            ...
            //print("[Date] setting trading Day")
            for day in days {
                if nextTradingDate == today,
                   today.closingHour <= today.timeComponents().hour { //after hours
                    nextTradingDate = day
                    continue
                } else if nextTradingDate.validTradingDay {
                    //print("[Date] found")
                    break
                } else {
                    nextTradingDate = day
    ...
    var validTradingDay: Bool {
        //print("[Date] \(self.dayofTheWeek)")
        return !self.dayofTheWeek.isWeekend && !self.isUSHoliday
    }
    
    static var lastValidTradingDay: Date? {
        if Date.today.validTradingDay,
           today.openingHour <= today.timeComponents().hour {
            return Date.today
        } else {
            for i in 1...12 {
                let date = Date.today.advanceDate(value: -1*i)
//                print("[Date] lastPrevious(\(i)) - \(date.asStringWithTime) // today: \(Date.today.dayofTheWeek.rawValue)")
