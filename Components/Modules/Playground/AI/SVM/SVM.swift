...
protocol SVMModelDelegate: class {
...
//    public static var supportsSecureCoding: Bool = true
    var context: StockServiceModels.Indicators.Context = .close
...
//        if pList == nil {type = .C_SVM_Classification; return nil }
    ...
    public func isνFeasableForData(data: DataSet) -> Bool
    {
        ...
        for i in 0..<data.size {
            if let index = label.firstIndex(of: data.classes![i]) {
                //  label already found
                count[index]+=1
            }
            else {
    ...
    public func train(data: DataSet)
    {
        ...
        case .OneClassSVM, .ϵSVMRegression, .νSVMRegression:
            ...
            if (probability && (type == .ϵSVMRegression || type == .νSVMRegression)) {
                probabilityA = [svrProbability(data: data)]
            }
            
            //  Train one set of support vectors
            let f = trainOne(data: data, costPositive: 0.0, costNegative: 0.0, display: true)
            ρ = [f.ρ]
        ...
        case .C_SVM_Classification, .ν_SVM_Classification:
            ...
            do {
                ...
                var weightedCost = [Double].init(repeating: Cost, count: classificationData.numClasses)
                ...
                var nonZero = [Bool].init(repeating: false, count: data.size)
                        ...
                        if let subProblem = DataSet(fromDataSet: data, withEntries: classificationData.classOffsets[i]) {
                            do {
                                ...
                                for index in 0..<classificationData.classCount[i] {
                                    try subProblem.setClass(index: index, newClass: 1)
                                }
                                ...
                                if (probability) {
                                    let result = binarySVCProbability(data: subProblem, positiveLabel: classificationData.foundLabels[i], costPositive: weightedCost[i], costNegative: weightedCost[j])
                                    probabilityA.append(result.A)
                                    probabilityB.append(result.B)
                                }
                                
                                //  Train on this sub-problem
                                let f = trainOne(data: subProblem, costPositive: weightedCost[i], costNegative: weightedCost[j], display: true)
                                functions.append(f)
                ...
                totalSupportVectors = 0
                supportVectorCount = [Int].init(repeating: 0, count: classificationData.numClasses)
                ...
                for _ in 0..<classificationData.numClasses-1 {
                    coefficients.append([Double].init(repeating: 0.0, count: totalSupportVectors))
                }
                var permutation = 0
                for i in 0..<classificationData.numClasses-1 {
                    for j in i+1..<classificationData.numClasses {
    ...
    private func trainOne(data: DataSet, costPositive : Double, costNegative : Double, display : Bool = false) -> DecisionFunction
    {
        ...
        case .C_SVM_Classification:
            ...
            solver?.delegate = self.delegate
            solver!.costPositive = costPositive
            solver!.costNegative = costNegative
            
            solver!.solveClassification(data: data, display: display)
            break
        case .ν_SVM_Classification:
            //  Instantiate a solver class
        ...
        case .OneClassSVM:
            ...
            solver!.solveOneClass(data: data, ν: ν, display: display)
            break
        case .ϵSVMRegression:
            //  Instantiate a solver class
            solver = Solver(kernelParams: kernelParams, ϵ: ϵ)
            ...
            solver!.solveRegression(data: data, p: p, display: display)
            break
        case .νSVMRegression:
            //  Instantiate a solver class
            solver = Solver_ν(kernelParams: kernelParams, ϵ: ϵ, ν: ν)
        ...
        if (display) {
            ...
            for index in 0..<data.size {
                ...
                {
                    numSupportVectors+=1
                    if let output = data.singleOutput(index: index) {
                        if(output > 0.0) {
    ...
    {
        var target = [Double].init(repeating: 0.0, count: data.size)
        ...
        if ((type == .C_SVM_Classification || type == .ν_SVM_Classification) && nFolds < data.size) {
            ...
            do {
                ...
                var foldCount = [Int].init(repeating: 0, count: nFolds)
                for i in 0..<nFolds {
                    for c in 0..<classificationData.numClasses {
...
//                        }
        ...
        for i in 0..<nFolds {
            ...
            if let subProblem = DataSet(fromDataSet: data, withEntries: subIndices) {
                ...
                subModel.train(data: subProblem)
                if (probability && (type == .C_SVM_Classification || type == .ν_SVM_Classification)) {
                    for j in foldStart[i]..<foldStart[i+1] {
                        target[perm[j]] = subModel.predictProbability(inputs: data.inputs[perm[j]])
                    }
                }
                else {
                    for j in foldStart[i]..<foldStart[i+1] {
                        target[perm[j]] = subModel.predictOne(inputs: data.inputs[perm[j]])
                    }
                }
            }
        }
    ...
    func binarySVCProbability(data: DataSet, positiveLabel: Int, costPositive: Double, costNegative: Double) -> (A: Double, B: Double)
    {
        ...
        var decisionValues = [Double].init(repeating: 0.0, count: data.size)
            ...
            if let subProblem = DataSet(fromDataSet: data, withEntries: subIndices) {
                ...
                else
                {
                    ...
                    for index in begin..<end {
                        decisionValues[perm[index]] = Double(subModel.predictOneFromBinaryClass(inputs: data.inputs[index]))
                    }
                }
            }
        }
        
        return sigmoidTrain(decisionValues: decisionValues, labels: data.classes!)
    }
    ...
    func svrProbability(data: DataSet) -> Double
    {
        ...
        probability = false
        var ymv = crossValidation(data: data, numberOfFolds: 5)
        ...
        for index in 0..<data.size {
            ...
            case .C_SVM_Classification, .ν_SVM_Classification:
                ...
                for sv in 0..<totalSupportVectors {
                    kernelValue.append(Kernel.calcKernelValue(parameters: kernelParams, x: data.inputs[index], y: supportVector[sv]))
                }
                
                //  Allocate vote space for the classification
                var vote = [Int].init(repeating: 0, count: numClasses)
    ...
    public func predictOne(inputs: [Double]) -> Double
    {
        ...
        var kernelValue: [Double] = []
        for sv in 0..<totalSupportVectors {
            kernelValue.append(Kernel.calcKernelValue(parameters: kernelParams, x: inputs, y: supportVector[sv]))
        }
    ...
    {
        let sum = predictOne(inputs: inputs)
        if (sum > 0) {
            return labels[0]
        }
        else {
    ...
    public func predictProbability(inputs: [Double]) -> Double
    {
        if ((type == .C_SVM_Classification || type == .ν_SVM_Classification) && probabilityA.count > 0 && probabilityB.count > 0) {
            ...
            for i in 0..<numClasses-1 {
                for j in i+1..<numClasses {
                    pairwiseProbability[i][j] = min(max(SVMModel.sigmoidPredict(decisionValue: data.outputs![0][k], probA: probabilityA[k], probB: probabilityB[k]), minProbability), 1.0-minProbability)
                    pairwiseProbability[j][i] = 1.0 - pairwiseProbability[i][j]
                    k+=1
                }
            }
            let probabilityEstimates = multiclassProbability(probabilityPairs: pairwiseProbability)
            var maxIndex = 0
            ...
            return predictOne(inputs: inputs)
        }
    }
    ...
    func multiclassProbability(probabilityPairs: [[Double]]) -> [Double]
    {
        ...
        for t in 0..<k {
            ...
            Q.append([Double].init(repeating: 0.0, count: k))
            for j in 0..<t {
        ...
        var pQp : Double
        var Qp = [Double].init(repeating: 0.0, count: k)
        while (iter < max_iter) {
...
internal class Solver {
    ...
    func solveClassification(data: DataSet, display : Bool = false)
    {
        ...
        α = [Double].init(repeating: 0.0, count: data.size)
        ...
        gradient = [Double].init(repeating: -1.0, count: problemData!.size)
    ...
    func solve()
    {
        ...
        while(iter < max_iter) {
            delegate?.SVMProgress(iter, max_iter)
            ...
            if let indexes = indices {
                ...
                updateAlphaStatus(index: i)
                updateAlphaStatus(index: j)
    ...
    func solveOneClass(data: DataSet, ν: Double, display : Bool = false)
    {
        ...
        for index in 0..<data.size {
            do {
                try data.setClass(index: index, newClass: 1)
            }
            catch {
        ...
        //  Initialize the gradients
        gradient = [Double].init(repeating: 0.0, count: problemData!.size)
    ...
    func solveRegression(data: DataSet, p: Double, display : Bool = false)
    {
        ...
        α = [Double].init(repeating: 0.0, count: data.size*2)
