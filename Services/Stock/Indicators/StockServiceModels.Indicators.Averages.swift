...
extension StockServiceModels.Indicators {
    ...
    func slicedVolatilities(_ days: Int = 4) -> [Volatility] {
        Array(volatilities.suffix(days))
    }
    ...
    func avgVolume(_ days: Int = 12) -> Double {
        history.suffix(days).map { $0.volumeValue }.reduce(0, +) / days.asDouble
    }
    ...
    func sma(_ days: Int = 12, context: Context) -> Double {
        let prefix = history.suffix(days)
    ...
    func ema(_ days: Int = 12, context: Context, limit: Int = 90, iter: Int = 0) -> Double {
        let firstOfHistoricalSecurities = history.last ?? security
        
        let indicatorsOfThePast = StockServiceModels.Indicators.init(firstOfHistoricalSecurities, with: self.history)
